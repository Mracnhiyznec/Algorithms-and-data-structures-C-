...
def main():
    choice = input("Введите 'r' чтобы сгенерировать точки случайно, введите 'm' чтобы ввести свои точки ").strip().lower()
    if choice == 'r':
        n = int(input("Количество случайных точек ").strip())
        pts = [R(random.randint(-100,100), random.randint(-100,100)) for _ in range(n)]
        print("Сгенерированные точки")
        for pt in pts:
            print(pt.x, pt.y)
    else:
        n = int(input("Введите количество точек ").strip())
        pts = []
        for _ in range(n):
